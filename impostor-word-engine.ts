...
export function sampleOne<T>(arr: T[], rng: RNG): T {
  if (!arr.length) throw new Error("sampleOne: empty array");
  const res = arr[Math.floor(rng() * arr.length)];
  if (res === undefined) throw new Error("sampleOne: undefined result");
  return res;
}
...
export type WordCard =
  | { kind: "secret"; word: string; category?: string }
  | { kind: "impostor" };
...
export type ClueEntry = {
  playerId: Id;
  clue: string; // stored normalized
  rawClue: string; // as given
  atMs: number;
};
...
export type EngineConfig = {
  ...
  // Validation rules
  enforceOneWord: boolean;      // true => exactly one "word token"
  forbidRepeatedClues: boolean; // true => no clue word repeats globally in game
  normalizeClues: "lower" | "none"; // default lower

  // Voting rules
  allowSkipVote: boolean;
  tiePolicy: "NO_ELIMINATION" | "ELIMINATE_NONE_BUT_ADVANCE"; // both mean no one eliminated; kept for explicitness
  requireAllVotes: boolean;     // true => wait all alive votes before resolving

  // Win conditions
  // - CREWMATES win when all impostors eliminated.
  // - IMPOSTORS win if they survive to maxRounds OR (optional) reach parity.
  impostorsWinOnSurviveToEnd: boolean; // if true, survive to maxRounds => IMPOSTORS win
  impostorsWinOnParity: boolean;       // if true, if impostors >= crewmates (alive) => IMPOSTORS win immediately

  // Optional impostor guess mechanic (single attempt per impostor)
  allowImpostorGuess: boolean;
  impostorGuessTiming: "AFTER_CLUES_BEFORE_VOTE" | "AFTER_ELIMINATION_REVEAL";
  impostorGuessOutcomeOnWrong: "NOTHING" | "ELIMINATE_GUESSER"; // if wrong, optionally eliminate the guesser

  // Suggested Improvements
  onlyGuessIfEliminated?: boolean; // If true, only the player eliminated in the REVEAL phase can guess
};
...
export type GameState = {
  ...
  forbiddenWords: string[]; // List of keywords that cannot be used as clues
...
export type EngineEvent =
  ...
  | { type: "TURN_CHANGED"; playerId: Id; state: GameState }
  | { type: "CLUE_ACCEPTED"; entry: ClueEntry; state: GameState }
  | { type: "CLUE_REJECTED"; reason: string; state: GameState }
  | { type: "VOTE_CAST"; entry: VoteEntry; state: GameState }
  | { type: "VOTE_REJECTED"; reason: string; state: GameState }
  | { type: "VOTE_RESOLVED"; eliminated: Id | null; tally: Record<string, number>; state: GameState }
  | { type: "IMPOSTOR_GUESS"; playerId: Id; guess: string; correct: boolean; state: GameState }
  | { type: "GAME_ENDED"; winner: WinSide; state: GameState };
...
export class ImpostorWordEngine {
  ...
  private cardsByPlayer: Record<Id, WordCard> = {};

  constructor(cfg: EngineConfig) {
    if (cfg.impostorCount < 1) throw new Error("impostorCount must be >= 1");
    if (cfg.maxRounds < 1) throw new Error("maxRounds must be >= 1");
  ...
  getPrivateCard(playerId: Id): WordCard {
    ...
    if (!card) throw new Error("Unknown playerId");
  ...
  submitClue(playerId: Id, clueRaw: string) {
    ...
    if (this.cfg.forbidRepeatedClues) this.state.usedClueSet[clue] = true;
  ...
  impostorGuess(playerId: Id, guessRaw: string) {
    if (!this.cfg.allowImpostorGuess) return;
    ...
    if (pl.role !== "IMPOSTOR") throw new Error("Only impostors can guess");
    if (pl.hasGuessed) throw new Error("This impostor already guessed");

    const timingOK =
      (this.cfg.impostorGuessTiming === "AFTER_CLUES_BEFORE_VOTE" && this.state.phase === "CLUE") ||
      (this.cfg.impostorGuessTiming === "AFTER_ELIMINATION_REVEAL" && this.state.phase === "REVEAL");

    if (!timingOK) throw new Error("Guess not allowed in current phase");
  ...
  castVote(voterId: Id, targetId: Id | "SKIP") {
    ...
    if (targetId !== "SKIP") this.ensureAlive(targetId);
    ...
    const entry: VoteEntry = { voterId, targetId, atMs: Date.now() };
  ...
  resolveVote() {
    ...
    for (const v of this.state.votesThisRound) {
      ...
      if (!alive.includes(v.voterId)) continue;
      if (v.targetId !== "SKIP" && !alive.includes(v.targetId)) continue;
    ...
    const eliminatedId = eliminatedKey === "SKIP" ? null : (eliminatedKey as Id);
  ...
  private emitTurn() {
    ...
    if (!pid) return; // edge case: game ended
  ...
  private isAlive(playerId: Id) {
    const pl = this.state.players.find(p => p.id === playerId);
    return !!pl?.alive;
  }

  private ensureAlive(playerId: Id, allowEliminated: boolean = false) {
    const pl = this.state.players.find(p => p.id === playerId);
    if (!pl) throw new Error("Unknown playerId");
    if (!pl.alive && !allowEliminated) throw new Error("Player is eliminated");
  }

  private ensurePhase(phase: Phase) {
    if (this.state.phase !== phase) throw new Error(`Invalid phase; expected ${phase}, got ${this.state.phase}`);
  }
  ...
  private validateClue(raw: string, normalized: string): string | null {
    if (!raw || !raw.trim()) return "Empty clue";
    ...
    if (this.cfg.enforceOneWord) {
      ...
      if (/\s/.test(trimmed)) return "Clue must be a single word (no spaces)";
      if (!/[A-Za-zÀ-ÿ0-9]/.test(trimmed)) return "Clue must contain letters or numbers";
    }

    // no repeat rule
    if (this.cfg.forbidRepeatedClues) {
      if (this.state.usedClueSet[normalized]) return "Clue already used";
    }

    // forbid exact secret word for crewmates
    if (this.eqLoose(normalized, this.state.secret.word)) return "Cannot say the secret word";
  ...
  private eqLoose(a: string, b: string) {
